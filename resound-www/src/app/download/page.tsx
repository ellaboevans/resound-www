...
const platforms = [
  {
    ...
    options: [
      {
        label: "Download DMG",
        meta: "Sonoma or later",
        href: "/resound/Resound.dmg",
        primary: true,
      },
      {
        label: "Universal build",
        meta: "M-series and Intel Macs",
        href: "/resound/Resound.dmg",
      },
    ],
    ...
    options: [
      {
        label: ".AppImage",
        meta: "Portable",
        href: "/resound/resound-linux.AppImage",
        primary: true,
      },
      {
        label: ".deb",
        meta: "Ubuntu, Debian, Mint",
        href: "/resound/resound-linux.deb",
      },
      {
        label: ".rpm",
        meta: "Fedora, RHEL",
        href: "/resound/resound-linux.rpm",
      },
      {
        label: "tar.gz",
        meta: "Any distro",
        href: "/resound/resound-linux.tar.gz",
      },
    ],
    ...
    options: [
      {
        label: "Download MSI",
        meta: "Recommended",
        href: "/resound/resound-windows.msi",
        primary: true,
      },
      {
        label: "Download EXE",
        meta: "Standalone installer",
...
function ApplePlatformIcon(props: Readonly<SVGProps<SVGSVGElement>>) {
  return (
    <svg
...
function LinuxPlatformIcon(props: Readonly<SVGProps<SVGSVGElement>>) {
  return (
...
function WindowsPlatformIcon(props: Readonly<SVGProps<SVGSVGElement>>) {
  return (
...
function DownloadHero() {
  return (
    <section className="mx-auto w-[min(1160px,calc(100%-2.5rem))] pt-42 pb-10 sm:pt-48 sm:pb-14">
      <div className="grid gap-10 lg:grid-cols-[0.95fr_0.7fr] lg:items-end">
        <div>
          <HeroReveal>
            <Badge className="mb-5 gap-2 bg-primary/10 text-primary">
              <span className="size-1.5 rounded-full bg-primary" />
              <span>Beta desktop builds</span>
            </Badge>
          </HeroReveal>
          ...
          <HeroReveal delay={0.1}>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-muted-foreground sm:text-xl">
              Resound brings a Dynamic Island-style music controller to your
              desktop. Choose the installer that matches your operating system.
            </p>
          </HeroReveal>
        </div>

        <HeroReveal delay={0.15}>
          <div className="grid gap-3 rounded-lg border bg-card/60 p-4 shadow-2xl shadow-black/20">
            <StatusRow icon={Cpu} label="Build channel" value="Public beta" />
            <StatusRow icon={Package} label="Latest macOS file" value="DMG" />
            <StatusRow icon={BadgeCheck} label="Price" value="Free" />
          </div>
        </HeroReveal>
      </div>
    </section>
...
function StatusRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-md border bg-background/50 px-4 py-3">
    ...
    <article id={platform.id} className="min-w-0">
      <div className="mb-4 flex items-end justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          {platform.comingSoon && (
            <Badge
              variant="outline"
              className="gap-1.5 border-muted-foreground/20 text-muted-foreground/60 text-[10px] font-semibold tracking-wide uppercase">
      ...
      <div className="grid gap-2.5">
        {platform.options.map((option) => (
          <Link
            key={`${platform.id}-${option.label}`}
            href={platform.comingSoon ? "#" : option.href}
            download={!platform.comingSoon}
            className={cn(
              "group flex min-h-12 items-center justify-between gap-3 rounded-lg border px-4 py-3 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
              platform.comingSoon
                ? option.primary
                  ? "pointer-events-none border-primary/30 bg-primary/30 text-primary-foreground/40"
                  : "pointer-events-none border-border/30 bg-background/15 text-muted-foreground/30"
                : option.primary
                  ? "border-primary bg-primary text-primary-foreground hover:bg-primary/90"
                  : "border-border bg-background/45 text-muted-foreground hover:border-primary/50 hover:text-foreground",
            )}>
...
function CommandBlock({ command }: Readonly<{ command: string }>) {
  ...
  return (
    <div className="mt-2 flex min-w-0 items-center gap-2 rounded-lg border bg-card/80 p-2">
      <pre className="min-w-0 flex-1 overflow-x-auto px-2 py-1 font-mono text-xs leading-5 text-muted-foreground">
        <code>{command}</code>
      </pre>
      <button
        type="button"
